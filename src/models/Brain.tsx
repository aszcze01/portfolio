...
const Brain = (props: JSX.IntrinsicElements['group']) => {
  ...
  let {isBioLinkHovered, isWorkLinkHovered, isContactLinkHovered} = snap

  const { nodes } = useGLTF(BrainScene) as GLTFResult

  const material = new THREE.MeshStandardMaterial({ color: 0x00ff00, opacity: 0.60, transparent: true })
  const material2 = new THREE.MeshStandardMaterial({ color: 0xffff00, transparent: false })

  useFrame((_, delta) => (group.current.rotation.y += delta))

  useEffect(() => {
    isBioLinkHovered && (
      brainstem.current.material = material2
    )
    isWorkLinkHovered && (
      rightHemisphere.current.material = material2
    )
    isContactLinkHovered && (
      leftHemisphere.current.material = material2
    )
